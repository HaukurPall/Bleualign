...
class Aligner:
    ...
    def __init__(self, options):
        ...
        if (
            not self.options["srctotarget"]
            and not self.options["targettosrc"]
        ...
        self.src, self.close_src = self._inputObjectFromParameter(self.options["srcfile"])
        self.target, self.close_target = self._inputObjectFromParameter(self.options["targetfile"])

        for f in self.options["srctotarget"]:
            obj, close_obj = self._inputObjectFromParameter(f)
    ...
    def mainloop(self):
        ...
        if multiprocessing_enabled and self.options["num_processes"] > 1:
            ...
            scorers = [
                AlignMultiprocessed(tasks, self.options, scores, self.log) for i in range(self.options["num_processes"])
            ]
            ...
            producer = multiprocessing.Process(
                target=tasks_producer,
                args=(
                    tasks,
                    num_tasks,
                    (self.src, self.srctotarget, self.target, self.targettosrc, self.options),
    ...
    def results(self):
        return self.out1, self.out2, self.all_multialigns
        ...
        while sourcegap or targetgap:
            ...
            if sourcegap and targetgap:
                ...
                if "galechurch" in self.options["gapfillheuristics"] and (
                    max(len(targetgap), len(sourcegap)) < 4
                    or max(len(targetgap), len(sourcegap)) / min(len(targetgap), len(sourcegap)) < 2
                ):
                    tempsrcgap = []
            ...
            for sourceid in range(source_len):
                if sourceid in bleualignsrc:
                    self.log(
                        ...
                        + str(sourceid)
                        + ": "
                        + str(self.bleualign[bleualignsrc.index(sourceid)][1])
                        + "\033[1;m"
                    )
                else:
                    ...
                    self.log("\033[1;31m" + str(sourceid) + ": unaligned. best cand " + str(bestcand) + "\033[1;m")

        if source_len and target_len:
            self.log(
                "\n"
                + str(len(self.bleualign))
                + " out of "
                + str(source_len)
                + " source sentences aligned by BLEU "
                + str(100 * len(self.bleualign) / float(source_len))
                + "%",
                2,
            )
            self.log(
                "after gap filling, "
                + str(multialignsrccount)
                + " out of "
                + str(source_len)
                + " source sentences aligned "
                + str(100 * multialignsrccount / float(source_len))
                + "%",
                2,
            )
            self.log(
                "after gap filling, "
                + str(multialigntargetcount)
                + " out of "
                + str(target_len)
                + " target sentences aligned "
                + str(100 * multialigntargetcount / float(target_len))
                + "%",
                2,
            )
    ...
    def printout(self, sourcelist, translist, targetlist):
        ...
        # Haukur hack
        self.all_multialigns.append(self.multialign)
        sentscores = {}
        ...
        for j, (src, target) in enumerate([i[0] for i in self.multialign]):
            ...
            if self.options["filter"] == "sentences":
                self.check_sentence_pair(
                    j, sources[-1], translations[-1], targets[-1], sources_output[-1], targets_output[-1], sentscores
                )
    ...
    def close_file_streams(self):
        ...
        if self.close_out_bad2:
            self.out_bad2.close()
        for should_be_closed, output_stream in zip(self.close_srctotarget, self.srctotarget):
            if should_be_closed:
                output_stream.close()
        for should_be_closed, output_stream in zip(self.close_targettosrc, self.targettosrc):
            if should_be_closed:
                output_stream.close()
